...
const {
  ...
  read_env_multi, read_container_name,
} = require('./lib/utils');
...
function main() {
  ...
  for (const wt of docker_worktrees) {
    ...
    const env = read_env_multi(wt_path, ['WORKTREE_ALIAS', 'WORKTREE_PORT_OFFSET']);
    ...
    const primary_base = config ? config.services.ports[config.services.primary] : null;
    const app_port = (offset !== '?' && primary_base !== null) ? primary_base + Number(offset) : '?';
    const domain_str = config ? (config_mod.domain_for(config, alias) || alias) : `${alias}.localhost`;

    rows.push({ alias, status, uptime, mem, cpu, app_port, domain: domain_str });
  }
  ...
  const header = `  ${pad('ALIAS', col.alias)}  ${pad('STATUS', col.status)}  ${pad('UPTIME', col.uptime)}  ${pad('MEMORY', col.mem)}  ${pad('CPU', col.cpu)}  ${pad('DOMAIN', col.domain)}`;
  ...
  for (const r of rows) {
    ...
    console.log(`  ${pad(r.alias, col.alias)}  ${status_padded}  ${pad(r.uptime, col.uptime)}  ${pad(r.mem, col.mem)}  ${pad(r.cpu, col.cpu)}  ${pad(r.domain, col.domain)}`);
  }
