...
function main() {
  ...
  if (config && config._repoRoot) {
    ...
    try {
      ...
      console.error('Could not determine repository root. Run this from inside a git repo or ensure wt.config.js is accessible.');
      process.exit(1);
    }
  }
  ...
  console.log(`Removed worktree: ${options.name}`);
}
