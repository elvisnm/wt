...
function main() {
  ...
  if (!name || name.startsWith('-')) {
    console.log('Usage:');
    console.log('  wt bash <name>    Open a bash shell inside the running container');
    console.log('');
    console.log('For a host shell at the worktree directory, use `wt shell <name>`.');
    process.exit(1);
  }
  ...
  if (shared) {
    ...
    try {
      ...
      if (state !== 'running') {
        console.error(`Container ${container} is ${state}. Start it with: wt up ${name}`);
        process.exit(1);
      }
    } catch {
      console.error(`Container ${container} not found. Start it with: wt up ${name}`);
      process.exit(1);
    }
  } else {
