...
Run isolated dev environments using Docker containers. Each worktree gets its own container, `.localhost` domain, and isolated configuration.
...
Text-based interactive hub. Shows existing worktrees, then lets you pick an action: create, manage (logs/restart/stop/bash/services), config (LAN toggle), or maintenance. No flags to memorize. No extra dependencies — runs on Node.js.
...
Step-by-step guided creation — prompts for branch, base ref, alias, database, and extras. Shows a summary with the equivalent `dc:up` command before executing. Also accessible from `pnpm dc` → "Create a new worktree".
...
pnpm dc:up feat/abc --shared-db                             # combine flags
...
- All configured services start automatically. Use `dc:service` to manage individual services on-demand
- If file watching is slow, use `--poll` to switch PM2 to polling mode
- Requires Docker Desktop (and shared infrastructure if configured)
