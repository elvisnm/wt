 ...
 * env_var, worktree_var, feature_enabled, get_compose_info.
 */
...
/**
 * Create a temp directory with a wt.config.js file.
...
function create_temp_config(config_obj) {
  ...
  const config_path = path.join(dir, 'wt.config.js');
...
describe('find_config', () => {
  ...
  test('stops at closest config file (does not continue walking up)', () => {
    ...
    const inner_config_path = path.join(inner_dir, 'wt.config.js');
...
describe('feature_enabled', () => {
  test('returns true for boolean true', () => {
    const cfg = { features: { lan: true } };
    expect(config_mod.feature_enabled(cfg, 'lan')).toBe(true);
  });

  test('returns false for boolean false', () => {
    const cfg = { features: { lan: false } };
    expect(config_mod.feature_enabled(cfg, 'lan')).toBe(false);
  });

  test('returns false for null', () => {
    const cfg = { features: { custom: null } };
    expect(config_mod.feature_enabled(cfg, 'custom')).toBe(false);
  });
  ...
  test('returns true for truthy non-boolean values (number)', () => {
    const cfg = { features: { custom: 2048 } };
...
describe('full integration: realistic config scenarios', () => {
  ...
  test('minimal config gets all defaults applied', () => {
    ...
    expect(cfg.dash.localDevCommand).toBe('npm start');
