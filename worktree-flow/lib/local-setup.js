 ...
 *
 * Extracted from wt-up.js to eliminate duplication between
...
function build_env_overrides(worktree_path) {
  ...
  const overrides = { NODE_ENV: 'development' };
 ...
 */
function generate_ecosystem(worktree_path, branch, port_offset) {
  const active_services = new Set(Object.keys(config.services.ports));
...
/**
 * Ensure default env vars (appUrl, localIp) are in the env file.
...
function start_build_watcher(worktree_path, branch, home, pm2_bin) {
  ...
  try {
    execSync(start_cmd, { stdio: 'inherit', cwd: worktree_path, env: { ...process.env, PM2_HOME: home } });
  } catch {
