 ...
 *
 * All port definitions come from wt.config.js. When no config is
 * loaded, exports are empty and callers must guard against null/empty values.
 */
...
function compute_ports(offset) {
  if (!config) {
    throw new Error('No wt.config.js found — cannot compute ports');
  }
  return config_mod.compute_ports(config, offset);
}

function format_port_table(offset) {
