...
function get_skip_paths() {
  ...
  if (paths.length === 0) {
    ...
    console.log('Add git.skipWorktree to your wt.config.js:');
    console.log('');
    console.log("  git: {");
...
function main() {
  ...
  if (!action || !['apply', 'remove', 'list'].includes(action)) {
    console.log('Usage:');
    console.log('  node wt-skip.js apply <name>   Apply skip-worktree to configured paths');
    console.log('  node wt-skip.js remove <name>  Remove skip-worktree from configured paths');
    console.log('  node wt-skip.js list <name>    List files with skip-worktree set');
