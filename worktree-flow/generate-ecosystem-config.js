 ...
 *
 * Reads all definitions from wt.config.js:
 *   services.ports, services.groups, services.pm2.debugPorts,
 *   services.pm2.envPassthrough
 *
...
function generate_config(config, worktree_path, worktree_name, port_offset, active_services, env_overrides) {
  ...
  for (const service_name of active_services) {
    ...
    if (source) {
      ...
      // but replace inspect with our computed value
      if (source.node_args) {
...
function main() {
  if (!config) {
    console.error('No wt.config.js found. Cannot generate ecosystem config.');
    process.exit(1);
  }

  if (!config.services.pm2 || !config.services.pm2.ecosystemConfig) {
    console.error('services.pm2.ecosystemConfig not set in wt.config.js');
    process.exit(1);
  }
