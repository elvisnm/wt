...
const os = require('os');
const { SERVICE_PORTS, compute_ports, format_port_table, find_free_offset } = require('./service-ports');
const { get_lan_ip, build_lan_domain } = require('./lan-ip');
const {
  config, config_mod, run, auto_alias, has_ref, compute_auto_offset,
  resolve_worktrees_dir, update_env_key,
} = require('./lib/utils');
const {
...
function ensure_prebaked_image() {
  ...
  const rebuild_script = path.join(scripts_dir, 'wt-rebuild-base.js');
...
function main() {
  ...
  if (fs.existsSync(worktree_path)) {
    ...
    if (options.no_docker) {
      ...
      const config_src = path.join(repo_root, 'wt.config.js');
      if (fs.existsSync(config_src)) {
        fs.copyFileSync(config_src, path.join(worktree_path, 'wt.config.js'));
      }

      // Ensure .env.worktree exists with alias
      const env_fn = config ? config.env.filename || '.env.worktree' : '.env.worktree';
      const env_fp = path.join(worktree_path, env_fn);
      if (!fs.existsSync(env_fp)) {
        const nv = config ? config_mod.worktree_var(config, 'name') || 'WORKTREE_NAME' : 'WORKTREE_NAME';
        const av = config ? config_mod.worktree_var(config, 'alias') || 'WORKTREE_ALIAS' : 'WORKTREE_ALIAS';
        fs.writeFileSync(env_fp, `${nv}=${path.basename(worktree_path)}\n${av}=${alias}\nWORKTREE_TYPE=local\n`);
      }
      ...
      if (local_setup.is_local_dev_enabled()) {
        ...
        local_setup.generate_ecosystem(worktree_path, target_branch, port_offset);
  ...
  if (options.no_docker) {
    ...
    // Copy wt.config.js so wt commands work inside the worktree
    const config_src = path.join(repo_root, 'wt.config.js');
    if (fs.existsSync(config_src)) {
      fs.copyFileSync(config_src, path.join(worktree_path, 'wt.config.js'));
    }
    ...
    // Always write basic .env.worktree for local worktrees (alias, name, type)
    const env_filename_local = config ? config.env.filename || '.env.worktree' : '.env.worktree';
    const env_path_local = path.join(worktree_path, env_filename_local);
    if (!fs.existsSync(env_path_local)) {
      const name_var = config ? config_mod.worktree_var(config, 'name') || 'WORKTREE_NAME' : 'WORKTREE_NAME';
      const alias_var = config ? config_mod.worktree_var(config, 'alias') || 'WORKTREE_ALIAS' : 'WORKTREE_ALIAS';
      const env_lines = [
        `${name_var}=${path.basename(worktree_path)}`,
        `${alias_var}=${alias}`,
        `WORKTREE_TYPE=local`,
      ];
      fs.writeFileSync(env_path_local, env_lines.join('\n') + '\n');
    } else {
      // Ensure alias is in existing env file
      const alias_var = config ? config_mod.worktree_var(config, 'alias') || 'WORKTREE_ALIAS' : 'WORKTREE_ALIAS';
      const env_content = fs.readFileSync(env_path_local, 'utf8');
      if (!env_content.includes(`${alias_var}=`)) {
        fs.appendFileSync(env_path_local, `${alias_var}=${alias}\n`);
      }
      if (!env_content.includes('WORKTREE_TYPE=')) {
        fs.appendFileSync(env_path_local, 'WORKTREE_TYPE=local\n');
      }
    }
    ...
    if (use_local_dev) {
      ...
      const port_offset = local_setup.resolve_offset(worktree_path);
      local_setup.ensure_env_defaults(worktree_path, alias);
      const { active_services, env_overrides } = local_setup.generate_ecosystem(worktree_path, target_branch, port_offset);
    ...
    if (use_local_dev) {
      ...
      const dev_cmd = config && config.dash && config.dash.localDevCommand ? config.dash.localDevCommand : null;
      if (!dev_cmd) {
        console.log('No dev command configured. Use the dashboard to build/start.');
      } else {
...
function create_git_worktree(repo_root, worktree_path, target_branch, options) {
  ...
  if (options.branch) {
    ...
    if (branch_exists_locally) {
      ...
      if (!base_ref) {
        console.error('Could not find a base branch. Configure repo.baseRefs in wt.config.js');
        process.exit(1);
      }
      execSync(
...
function create_shared(repo_root, worktree_path, target_branch, alias, options) {
  ...
  const offset_var = config_mod.worktree_var(config, 'portOffset') || 'WORKTREE_PORT_OFFSET';
...
function restart_generate(repo_root, worktree_path, target_branch, alias, env_file, options) {
  ...
  execSync(
    `node "${compose_script}" --path "${worktree_path}" --name "${target_branch}" --alias "${alias}" --offset ${restart_offset}${lan_flag}`,
    { stdio: 'inherit' },
  );
  ...
  const recreate_flag = (options.rebuild || options.lan) ? ' --force-recreate' : '';
...
function create_generate(repo_root, worktree_path, target_branch, alias, env_file, options) {
  ...
  const compose_script = path.join(scripts_dir, 'generate-docker-compose.js');
  execSync(
    `node "${compose_script}" --path "${worktree_path}" --name "${target_branch}" --alias "${alias}" --offset ${port_offset}${lan_flag}`,
    { stdio: 'inherit' },
  );
  ...
  if (has_local_db() && !options.shared_db) {
    ...
    if (db_name_str) {
      console.log('');
      console.log(`Database: ${db_name_str}`);
    }
  }
}
