...
const debug = make_debug('wt-create.js');
...
async function main() {
  ...
  if (action === 'restart') {
    ...
    const up_script = path.join(scripts_dir, 'wt-up.js');
  ...
  if (use_docker) {
    ...
    debug('main: calling wt-up: node ' + up_script + ' ' + args.join(' '));
    execSync(`node "${up_script}" ${args.map((a) => `"${a}"`).join(' ')}`, {
      stdio: 'inherit',
      cwd: repo_root,
    });
    debug('main: wt-up completed (docker)');
  } else {
    const flags = [`--from=${from_ref}`, '--no-docker', `--alias=${final_alias}`];
    if (db_choice === 'shared') flags.push('--shared-db');
