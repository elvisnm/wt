...
func build_detail_lines(wt *worktree.Worktree, inner_w, spin_frame int, cfg *config.Config) []string {
	...
	if wt.Type == worktree.TypeDocker {
		...
		// Service Ports
		lines = append(lines, "")
		lines = append(lines, lipgloss.NewStyle().Foreground(lipgloss.Color("248")).Render("Ports"))
		...
		// App URL — show domain when available, show localhost:port when config is available
		if wt.Domain != "" {
...
func build_port_lines(wt *worktree.Worktree, cfg *config.Config) []string {
	...
	// No config — cannot show ports
	return []string{lipgloss.NewStyle().Foreground(DimTextColor).Render("  (no config)")}
}
