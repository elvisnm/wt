...
pub fn render_help(frame: &mut Frame, area: Rect) {
    ...
    let left: Vec<Vec<Span>> = [
        ...
        vec![box_line(col_w, b, vec![s("  ", d), s("z", k), s("           shell (host, worktree dir)", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("b", k), s("           bash (container, when running)", d)])],
    ...
    let right: Vec<Vec<Span>> = [
        ...
        vec![vec![]],
        vec![box_line(col_w, b, vec![s("  Opens tasks list + DAG graph.", d)])],
        vec![vec![]],
        vec![box_line(col_w, b, vec![s("  ", d), s("j", k), s(" / ", d), s("k", k), s("       navigate tasks", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Enter", k), s("       expand tooltip", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Esc", k), s("         collapse tooltip", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+n", k), s("      new task", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+c", k), s("      close task", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+d", k), s("      delete task", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+s", k), s("      search/filter", d)])],
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+e", k), s("      edit task", d)])],
        vec![box_bottom(col_w, b)],
        vec![box_top("DAG Graph (attached)", col_w, b, t)],
        vec![vec![]],
        ...
        vec![box_line(col_w, b, vec![s("  ", d), s("Ctrl+G", k), s("      overlays toggle", d)])],
