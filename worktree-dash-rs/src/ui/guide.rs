...
pub fn render_guide(frame: &mut Frame, area: Rect) {
    ...
    let rows: Vec<(Vec<Span>, Vec<Span>)> = vec![
        ...
        (
            ...
            box_line(col_w, b, vec![s("  ", d), s("Tab", k), s("         next panel", d)]),
        ),
        (box_empty(col_w, b),
        ...
        (
            ...
            box_line(col_w, b, vec![s("    ", d), s("s", k), s("  services", d)]),
        ),
        (
            box_empty(col_w, b),
        ...
        // Terminal + More
        (box_top("Terminal", col_w, b, t), box_top("More", col_w, b, t)),
        (box_empty(col_w, b), box_empty(col_w, b)),
        (
        ...
        (
            box_line(col_w, b, vec![s("    ", d), s("Ctrl+j/k", k), s("  switch session", d)]),
            box_empty(col_w, b),
        ),
        (box_bottom(col_w, b), box_bottom(col_w, b)),
        // Tabs + Actions
        (box_top("Tabs", col_w, b, t), box_top("Actions", col_w, b, t)),
        (box_empty(col_w, b), box_empty(col_w, b)),
        (
