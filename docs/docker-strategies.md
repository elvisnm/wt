...
1. `wt-up.js` calls `generate-docker-compose.js`
...
```js
services: {
  ports: { app: 3001, api: 3004, worker: 3005 },
  primary: 'app',
},
```

All defined services start when a worktree is created. PM2 manages them at runtime — use `dc:service` to start/stop individual services on demand.
...
1. `wt-up.js` computes a port offset and branch slug
